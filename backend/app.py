...
def login():
    with User() as c_user:
        ...
        # print resp
        return jsonify(id_user=resp[0],nama=resp[1],jenis_kelamin=resp[2],email=resp[3],universitas=resp[4],no_ktm=resp[5],status_aktif=resp[6],status_akses=resp[7],status='1',no_hp=resp[9])
...
def getLombaSaya():
    with Lomba() as c_lomba:
        ...
        list_lomba = [{"id_adm":res[0],"id_lomba":res[1],"nama_lomba":res[2],"deadline":res[3],"nama_tim":res[4],"id_pembuat":res[5]} for res in resp]
        return jsonify(list_lomba)
